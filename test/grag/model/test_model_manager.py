...
def run_tests():
    """运行所有测试"""
    print("\n" + "=" * 60)
    print("开始测试 ModelManager 模块")
    print("=" * 60)

    test_mgr = TestModelManager()
    test_init = TestModelManagerInitialize()
    test_global = TestModelManagerGlobalFunctions()

    try:
        test_mgr.setup_method()
        test_mgr.test_model_manager_init()

        test_mgr.setup_method()
        test_mgr.test_get_llm_client()

        test_mgr.setup_method()
        test_mgr.test_get_llm_client_with_provider()

        test_mgr.setup_method()
        test_mgr.test_get_embedding_client()

        test_mgr.setup_method()
        test_mgr.test_get_embedding_client_with_provider()

        test_mgr.setup_method()
        test_mgr.test_get_reranker_client()

        test_mgr.setup_method()
        test_mgr.test_get_embedding_dimension()

        test_mgr.setup_method()
        test_mgr.test_get_model_info()

        test_mgr.setup_method()
        test_mgr.test_get_model_status()

        test_mgr.setup_method()
        test_mgr.test_refresh_all_models()

        test_mgr.setup_method()
        test_mgr.test_refresh_model_single()

        test_mgr.setup_method()
        test_mgr.test_cleanup()

        test_init.setup_method()
        test_init.test_initialize_models()

        test_global.setup_method()
        test_global.test_get_model_manager()

        test_global.setup_method()
        test_global.test_global_get_llm_client()

        test_global.setup_method()
        test_global.test_global_get_embedding_client()

        test_global.setup_method()
        test_global.test_global_get_model_info()

        test_global.setup_method()
        test_global.test_global_get_model_status()

        test_global.setup_method()
        test_global.test_global_get_embedding_dimension()
