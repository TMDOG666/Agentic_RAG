...
这个文件演示如何构建一个完全支持 Agent Skills 标准的智能体系统。

核心功能：
1. 自动扫描和加载 Skills 目录
2. 三层渐进式加载机制（元数据 → 完整指令 → 额外文件）
3. 大模型主动判断并按需加载 Skills
4. 支持 Skills 中的可执行脚本

技术栈：
- LangGraph: 构建有状态的 Agent 工作流
- LangChain: 工具调用和模型集成
- SiliconFlow API: 提供 LLM 和 Embedding 服务
- Chroma: 向量数据库（用于 RAG）
...
# 启动时一次性完成 runtime 组装：
# - SkillManager（扫描 skills）
# - Tools（function calling）
# - LLM 模型（按配置/环境变量创建）
# - LangGraph 工作流（agent-tools 循环）
#
# 这样做的好处是：
# - `run_once()` 每次调用只需要驱动 GRAPH，不用重复初始化
# - CLI 启动时即可尽早暴露配置/依赖问题（例如缺少 langgraph）
