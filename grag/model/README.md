...
```python
from grag.model import initialize_models, get_model_manager
...
# 使用重排序（如果可用）
if manager.get_reranker_client():
    reranked_docs = manager.rerank_documents("query", ["doc1", "doc2", "doc3"])
```

### 高级功能

```python
from grag.model import get_llm_client, get_embedding_client
